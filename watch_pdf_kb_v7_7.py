...
DATA_DIR = ROOT / "data"
DB_PATH = DATA_DIR / "watch_ai_pdf_web_kb.sqlite3"
...
def _chunk_text(text: str, chunk_size: int = 950, overlap: int = 120) -> list[str]:
...
def extract_pdf_text(pdf_bytes: bytes) -> str:
    ...
    raise RuntimeError("无法抽取 PDF 文字。请安装：pip install pypdf。详情：" + " | ".join(errors))
...
def add_pdf_bytes(filename: str, pdf_bytes: bytes) -> tuple[bool, str]:
    ...
    title = filename or hashlib.sha256(pdf_bytes).hexdigest()[:16]
...
def _tokenize_query(query: str) -> list[str]:
    ...
    for kw in [
        "算力", "租赁", "智算", "AI", "人工智能", "数据中心", "主营", "公告",
        "合同", "业绩", "风险", "首板", "打板", "半路", "涨停", "题材",
        "订单", "减持", "增持", "监管", "问询", "并购", "重组",
    ]:
...
def search_kb(query: str, top_k: int = 8) -> list[dict[str, Any]]:
    ...
    with connect() as conn:
        rows = [dict(r) for r in conn.execute("SELECT * FROM chunks ORDER BY created_at DESC LIMIT 3000").fetchall()]
